                     ...
                     <table class="table table-hover" >
                         <tbody id="dataTable">
                            ...
                            <c:forEach items="${diskList}" var="disk">
                                <tr class="tr" rel="${disk.id}" nihao="${disk.type}">
                                    ...
                                    <td>${disk.name}</td>
                                    <td><fmt:formatDate value="${disk.updateTime}" dateStyle="short" /> </td>
                                    <td width="100">
                                    ...
                                    <td width="150">
                                        <div class="btn-group">
                                            ...
                                            <ul class="dropdown-menu">
                                                ...
                                                <c:if test="${disk.type == 'file'}">
                                                    <li><a href="/disk/download?_=${disk.id}">下载</a></li>
                                                </c:if>
                                                <li><a class="rename" href="javascript:;" rel="${disk.id}">重命名</a></li>
    ...
    <tr class="tr" rel="{{id}}">
        ...
        <td width="100">
            <div class="btn-group">
                ...
                <ul class="dropdown-menu">
                    <? if(type == 'file') { ?>
                    <li><a href="/disk/download?_={{id}}">下载</a></li>
                    <? } else if(type == 'dir') { ?>
                    <li><a href="/disk?_={{id}}">打开</a></li>
                    <?}?>
                    <li><a class="rename" href="javascript:;" rel="{{id}}">重命名</a></li>
...
<script>
    $(function () {
                ...
                $.post("/disk/new/folder",{"pid":pid,"name":text,"accountId":accountId}).done(function(resp){
                    if(resp.state == 'success') {
                        ...
                        for(var i = 0;i < resp.data.length;i++) {
                            ...
                            obj.updateTime = moment(obj.updateTime).format("YY-MM-DD"); //将时间戳格式化
        ...
        $(document).delegate(".rename","click",function (event) {
            ...
            layer.prompt({title:"请输入新的文件夹名称"},function(text,index){
                layer.close(index);
                $.post("/disk/rename",{"id":id,"accountId":accountId,"name":text}).done(function(resp){
                    if(resp.state == 'success') {
                       window.history.go(0);
                    } else {
                        layer.msg(resp.message);
